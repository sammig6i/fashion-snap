        ...
        .map((wrapper) => {
          ...
          if (sourceTags.length > 0) {
            ...
            if (srcset) {
              ...
              return firstImageUrl.replace(/(\?|&)w=\d+(&|$)/, "$1w=2048$2")
            }
          }
          return null
        })
    ...
    const gridImages = Array.from(productCards)
      .map((card) => {
        ...
        if (img && img.src && !img.src.includes("transparent-background.png")) {
          return img.src.replace(/(\?|&)w=\d+(&|$)/, "$1w=2048$2")
        }
        return null
      })
      .filter(Boolean) as string[]
...
async function sendImagesToBackground() {
  ...
  if (pageType !== "unknown") {
    ...
    if (newImages.length > 0) {
      try {
        ...
        if (response.filteredImages) {
          await sendToBackground({
            name: "updateImages",
            body: {
              images: response.filteredImages
            }
          })
